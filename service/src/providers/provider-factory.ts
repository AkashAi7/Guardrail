...
export class ProviderFactory {
  ...
  private static async createCopilotProvider(config: ProviderConfig): Promise<CopilotProvider> {
    try {
  ...
  private static async createBYOKProvider(config: ProviderConfig): Promise<BYOKProvider> {
    try {
      if (!config.byok) {
  ...
  static async getAvailableProviders(): Promise<{
    ...
    let recommended: 'copilot' | 'byok' | 'none' = 'none';
    if (copilotAvailable) {
