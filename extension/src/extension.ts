...
function getBatchAnalysisTimeout(documents: vscode.TextDocument[]): number {
...
async function analyzeDocumentsWithFallback(
    activeServiceManager: ServiceManager,
    documents: vscode.TextDocument[]
): Promise<{ results: any[]; failedFiles: string[] }> {
    const requestBatch = async (batchDocuments: vscode.TextDocument[]): Promise<any[]> => {
    ...
    const scanProjectCmd = vscode.commands.registerCommand('codeGuardrail.scanProject', async () => {
        ...
        await vscode.window.withProgress({
            ...
            vscode.window.showInformationMessage(
                `🤖 Project scan complete! Analyzed ${analyzed} files, found ${totalIssues} issues${failedFiles > 0 ? `, ${failedFiles} failed` : ''}.`,
                'View Issues'
            ).then(selection => {
...
`;
    ...
    const uploadComplianceDocumentCmd = vscode.commands.registerCommand('codeGuardrail.uploadComplianceDocument', async () => {
        try {
            ...
            await vscode.window.withProgress({
                ...
                if (response.success) {
                    vscode.window.showInformationMessage(
                        `✅ Uploaded compliance document: ${documentName} (${selectedType.label})`
                    );
    ...
    const viewComplianceDocumentsCmd = vscode.commands.registerCommand('codeGuardrail.viewComplianceDocuments', async () => {
        try {
            ...
            const documents = response.documents as Array<{ key: string; type: string; documentName: string }>;
    ...
    const clearComplianceDocumentsCmd = vscode.commands.registerCommand('codeGuardrail.clearComplianceDocuments', async () => {
        try {
            ...
            const documents = listResponse.documents as Array<any> || [];
            ...
            const confirmation = await vscode.window.showWarningMessage(
                `Clear all ${documents.length} compliance document(s)? This will remove the compliance context from analysis.`,
                { modal: true },
                'Clear All'
            );
